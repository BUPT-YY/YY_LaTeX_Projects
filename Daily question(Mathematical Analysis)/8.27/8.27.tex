...
\questiontext{
	证明:群$G$为交换群当且仅当$x\mapsto x^{-1},x\in G$是一个自同构映射.
}
\answer{
	\begin{proof}
		由同构映射的定义,得到
		\begin{equation*}
			(x\circ y)^{-1} = x^{-1}\circ y^{-1},\forall x,y\in G.
		\end{equation*}
		于是,\begin{equation*}
			\begin{split}
				x\circ y &= (x^{-1}\circ y^{-1})^{-1} \\
				&= y\circ x,\forall x,y\in G.
			\end{split}
		\end{equation*}
		反过来, $G$交换说明$x\circ y = y\circ x, \forall x,y\in G$.
		这推出了\begin{equation*}
			(x\circ y)^{-1} = x^{-1}\circ y^{-1},\forall x,y\in G.	
		\end{equation*}
			这说明此映射是同态映射.
			
		不难看出,对每$a\in G$, 在$G$中都有逆元$a^{-1}$, 而$a^{-1}\mapsto (a^{-1})^{-1} = a$.
		这说明这个映射是满射.
		
		而$x \neq y\Rightarrow x^{-1}\neq y^{-1}$. 这说明这个映射是单射.
		
		因此,$x\mapsto x^{-1},x\in G$是一个既单又满的同态, 即证.
	\end{proof}
