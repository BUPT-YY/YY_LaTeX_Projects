...
%----------------------------------------------------------------------------------------
\newtheorem*{definition}{\color{blue}定义}
\begin{document}

\begin{question}

\questiontext{
	{\kaishu{集合$S$上的一个\myind{二元代数运算}是指$S\times S$到$S$的一个映射.}}
	\begin{definition}[群]
		设$G$是一个非空集合.如果在$G$上定义了一个二元运算$\circ$,通常称为乘法,并且满足:\\
		(1)~\myind{结合律}: $(a\circ b)\circ c = a\circ (b\circ c), \forall a,b,c\in G$;\\
		(2)存在\myind{幺元}: 存在$e\in G$,使得$\begin{lgathered}
			e\circ a = a\circ e = a, \forall a\in G
			\end{lgathered}$($e$称为$G$的幺元);\\
		(3)存在\myind{逆元}: 对每$a\in G$,存在$b\in G$,使得$\begin{lgathered}
			a\circ b = b\circ a = e
			\end{lgathered}$($b$称为$a$的逆元,记作$a^{-1}$),\\
		则称$G$关于运算$\circ$构成一个\myind{群},记为$(G,\circ)$,或者简记为$G$.
	\end{definition}
	试证:群的定义可以简化为:如果一个非空集合$G$上定义了一个二元运算$\circ$,满足:\\
	(1)~\myind{结合律}: $(a\circ b)\circ c = a\circ (b\circ c), \forall a,b,c\in G$;\\
	(2)存在\myind{左幺元}: 存在$e\in G$,使得$\begin{lgathered}
	e\circ a = a, \forall a\in G
	\end{lgathered}$;\\
	(3)存在\myind{左逆元}: 对每$a\in G$,存在$b\in G$,使得$\begin{lgathered}
		b\circ a = e
	\end{lgathered}$,\\
	则$G$关于运算$\circ$构成一个群.
}
\answer{
	\begin{proof}
		...
		对任意$a\in G$, 有$b \in G$,使$b\circ a = e$, 由于$b\in G$, 所以$b$在$G$中也有左逆元, 即存在$c\in G$, 使$c\circ b = e$.
		
		因此,$
			a\circ b = e\circ (a\circ b) = (c\circ b) \circ (a\circ b) =  c\circ ((b \circ a)\circ b) = c\circ b = e.
		$
		
		再接着证明左幺元也是右幺元:
		
		对任意$a\in G$,有$b \in G$,使$b\circ a = e$.
		
		由此, $a\circ e = a\circ (b\circ a) = (a\circ b)\circ a = a.$
	\end{proof}
}
