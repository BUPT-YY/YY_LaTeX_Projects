...
\begin{document}
	...
	\begin{homeworkProblem}
		...
		\end{equation}
		\begin{proof}
			一方面,设$\omega\in \bigcup_{n=1}^{+\infty}A_n'$, 根据并运算的定义知道:$\exists n\in\mathbb{N}^*(\omega\in A_n').$
			注意到$A_n'\subset A_n$, 所以$\omega\in A_n'\Rightarrow \omega\in A_n$.
			因此,有$\exists n\in\mathbb{N}^*(\omega\in A_n).$
			这说明\begin{equation}
				\bigcup_{n=1}^{+\infty}A_n' \subset \bigcup_{n=1}^{+\infty}A_n.
			\end{equation}
			令一方面, 设$\omega\in \bigcup_{n=1}^{+\infty}A_n$, 根据并运算的定义知道:$\exists n\in\mathbb{N}^*(\omega\in A_n).$
			
			这里为了讨论方便, 引入$A_0 = \varnothing$.
			
			此时, 先看$\omega$是否属于$A_1$, 若$\omega\in A_1$, 则$\omega\in A_1'$; 若$\omega\notin A_1$, 则继续看$\omega$是否属于$A_2$. 若$\omega\in A_2$, 则$\omega\in A_2\backslash A_1 = A_2'$. 若$\omega\notin A_2$, 则继续看$\omega$是否属于$A_3$,
			若$\omega\in A_3$, 则$\omega\in A_3\backslash(A_1\cup A_2) = A_3'$, 否则继续看$\omega$是否属于$A_4$,以此类推. 因此, 必有整数$k\in\{1,\cdots,n\}$使得$\omega\notin A_0,\cdots A_{k-1}$, 但$\omega\in A_k$.即$\omega\in A_k'$.
	...
	\begin{homeworkProblem}
		...
		\begin{align}
			&\bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}A_k = \{ \omega:\omega\text{属于无穷多个}A_n \},\nonumber\\
			&\bigcup_{n=1}^{+\infty}\bigcap_{k=n}^{+\infty}A_k = \{ \omega:\omega\text{只不属于有限多个}A_n \}.
		\end{align}
		\begin{proof}
			(1)若$\omega\in\bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}A_k$, 则根据交的定义知任给$n\in\mathbb{N}^*$, 有\begin{equation}
				\omega\in\bigcup_{k=n}^{+\infty}A_k
			\end{equation}
			从而根据并的定义又知道:存在$k\geqslant n$, 使$\omega\in A_k$.
			
			即\begin{equation}
				\forall n\in\mathbb{N}^*,\exists k\geqslant n~\left( \omega\in A_k \right).
			\end{equation}
			这说明$\omega\in\bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}A_k$当且仅当$\omega$属于集合列$\{A_j\}$中的无穷多个集合.
			...
			\end{equation}
			这说明$\omega\in\bigcup_{n=1}^{+\infty}\bigcap_{k=n}^{+\infty}A_k$当且仅当$\omega$只不属于集合列$\{A_j\}$中的有限多个集合.
		\end{proof}
	
		\textbf{Note}用概率论的语言, 可以把这个结论总结为:
		\begin{enumerate}
			\item 事件列$\{A_j\}$的上极限发生当且仅当$\{A_j\}$中有无穷个事件发生;
			\item 事件列$\{A_j\}$的下极限发生当且仅当$\{A_j\}$中仅有有限个事件不发生.
		\end{enumerate}
	...
	\begin{homeworkProblem}
		...
		\begin{proof}
			\begin{enumerate}
				\item 根据习题2知道: $\omega\in\bigcup_{n=1}^{+\infty}\bigcap_{k=n}^{+\infty}A_k$当且仅当$\omega$只不属于集合列$\{A_j\}$中的有限多个集合. 这可以推出 $\omega$属于集合列$\{A_j\}$中的无穷多个集合, 即$\omega\in\bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}A_k$. 这说明$\bigcup_{n=1}^{+\infty}\bigcap_{k=n}^{+\infty}A_k\subset\bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}A_k$.
				...
				\end{equation}
				再根据$\{A_n\}$单调非降, 得到$\bigcap_{k=n}^{+\infty}A_k = A_n$.
				这说明$\liminf A_n = \cup_{n=1}^{+\infty}A_n$. 
				
				结合上面的包含关系知道结论成立.
				\item 根据$\{A_n\}$单调非增, 得到$\bigcup_{k=n}^{+\infty}A_k = A_n$.
				这说明$\limsup A_n = \cap_{n=1}^{+\infty}A_n$.
				
				结合包含关系(11)可知结论成立.
				\item 回忆de Morgen法则:
				\begin{align}
					&A\backslash \bigcup_{\lambda\in\Lambda}B_{\lambda} = \bigcap_{\lambda\in\Lambda}(A\backslash B_{\lambda})\nonumber\\
					&A\backslash \bigcap_{\lambda\in\Lambda}B_{\lambda} = \bigcup_{\lambda\in\Lambda}(A\backslash B_{\lambda}).
				\end{align}
				我们由此知道
				\begin{align}
					A\backslash \liminf_{n\to\infty}A_n &= A\backslash \bigcup_{n=1}^{+\infty}\bigcap_{k=n}^{+\infty}A_k\nonumber\\
					&=\bigcap_{n=1}^{+\infty}\left( A\backslash \bigcap_{k=n}^{+\infty}A_k \right)\nonumber\\
					&= \bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}\left( A\backslash A_k \right)\nonumber \\
					&=\limsup_{n\to\infty}(A\backslash A_n),
				\end{align}
				和\begin{align}
				A\backslash \limsup_{n\to\infty}A_n &= A\backslash \bigcap_{n=1}^{+\infty}\bigcup_{k=n}^{+\infty}A_k\nonumber\\
				&=\bigcup_{n=1}^{+\infty}\left( A\backslash \bigcup_{k=n}^{+\infty}A_k \right)\nonumber\\
				&= \bigcup_{n=1}^{+\infty}\bigcap_{k=n}^{+\infty}\left( A\backslash A_k \right)\nonumber \\
				&=\liminf_{n\to\infty}(A\backslash A_n).
				\end{align}
			\end{enumerate}
		\end{proof}
	\end{homeworkProblem}
	...
	\begin{homeworkProblem}
		...
		\begin{proof}
			...
			\begin{align}
				&\bigcap_{n=1}^{+\infty}\left(-\infty,a+\frac{1}{n}\right) = (-\infty,a]\nonumber\\
				&\bigcup_{n=1}^{+\infty}\left(-\infty,a-\frac{1}{n}\right] = (-\infty,a)
			\end{align}
	...
	\begin{homeworkProblem}
		求包含二集合$A,B$的最小$\sigma$-代数, 其中$A,B\neq\varnothing$且$\Omega,AB\neq\varnothing, A\cup B\neq\Omega$, 而且$A,B$互不包含.
		
		\solution
		所求的集合系为:\begin{equation}
		    \mathscr{A} = \{\varnothing,A,B,A\cup B,A\Delta B,A\backslash B, B\backslash A,\Omega,A^c,B^c,A^cB^c,AB^c,BA^c,(AB)^c,(A\Delta B)^c \}
		\end{equation}
	\end{homeworkProblem}
	...
	\begin{homeworkProblem}
		...
		\solution
		引入\begin{equation}
			A_0 = \Omega\backslash\bigcup_{k=1}^{+\infty}A_k.
		\end{equation}
		则所求为:
		\begin{equation}
		...
		\end{equation}
		\begin{proof}
		    不妨记(19)式右端为$\mathscr{D}$. 由于$\sigma$代数是对于可列并的运算以及补的运算封闭的, 故$\sigma(\mathscr{G}) \supset \mathscr{D}$. 因此, 要完成这个定理的证明, 只需证$\sigma(\mathscr{G}) \subset \mathscr{D}$. 为此, 又只需证$\mathscr{D}$是一个$\sigma$-代数. 下面, 分别验证$\sigma$-代数的三个条件:
		    \begin{enumerate}[1)]
		        \item \begin{equation}
		            \Omega = \bigcup_{k=0}^{+\infty}A_k \in \mathscr{D};
		        \end{equation}
		        \item \begin{equation}
		            A = \bigcup_{k\in K}A_{k}\in \mathscr{D} \Rightarrow A^c = \bigcup_{k\in (\mathbb{N}\backslash K)}A_{k} \in\mathscr{D};
		        \end{equation}
		        \item 设$B_1,B_2,\cdots \in \mathscr{D}$, 则存在$K_1,K_2,\cdots \subset \mathbb{N}$, 使\begin{equation}
		            B_n = \bigcup_{k\in K_n}A_{k}, n=1,2,\cdots.
		        \end{equation}
		        这时,\begin{equation}
		            \bigcup_{n=1}^{+\infty}B_n = \bigcup_{n=1}^{+\infty}\bigcup_{k\in K_n}A_{k} = \bigcup_{k\in K}A_{k}\in\mathscr{D}.
		        \end{equation}
		        其中$K = \cup_{n=1}^{+\infty}K_n$.
		    \end{enumerate}
		    由此知道$\mathscr{D}$确是一个$\sigma$-代数.
		\end{proof}
	\end{homeworkProblem}
	...
	\begin{homeworkProblem}
		...
		\begin{proof}
		    ...
		    \begin{enumerate}[1)]
		        ...
		        \item 设$A\in\mathscr{A}$, 则$A$是$\Omega$的至多可列子集或 以$\Omega$的至多可列子集为余集的子集.
		        这两类情况下, $A^c$分别是$\Omega$的以至多可列子集为余集的子集 或 $\Omega$的至多可列子集, 因此, $A^c\in\mathscr{A}$;
                                                                                     		        
		        \item 设$A_1,A_2,\cdots \in \mathscr{A}$. 有下列两种情况:\\
		        (1) 若诸$A_j$均是$\Omega$的至多可列子集, 则$\cup_j A_j$仍是$\Omega$的至多可列子集;\\
		        (2) 若诸$A_j$中至少有一个是 以$\Omega$的至多可列子集为余集的子集, 则$\cup_j A_j$仍是以$\Omega$的至多可列子集为余集的子集.
		        
		        所以总有\begin{equation}
		            \bigcup_{n=1}^{+\infty}A_n\in\mathscr{A}
		        \end{equation}
		    \end{enumerate}
		    由此知道$\mathscr{A}$确是一个$\sigma$-代数.
		\end{proof}
	\end{homeworkProblem}
	...
	\begin{homeworkProblem}
		...
		\begin{proof}
		    ...
		    \begin{enumerate}[1)]
		        ...
		        \item 设$E_1,E_2,\cdots \in \sigma(\mathscr{G})\cap A$. 即$E_j = AB_j$, 其中$B_j\in\sigma(\mathscr{G}),j = 1,2,\cdots$. 由于$\sigma(\mathscr{G})$是一个$\sigma$-代数, 所以$\cup_{j=1}^{+\infty}B_j\in\sigma(\mathscr{G})$. 这说明
		        \begin{equation}
		            \bigcup_{j = 1}^{+\infty}E_j = \bigcup_{j = 1}^{+\infty}AB_j  =A\left(\bigcup_{j = 1}^{+\infty}B_j \right)\in \sigma(\mathscr{G})\cap A.
		        \end{equation}
		    \end{enumerate}
		    再设$\mathscr{E}$是以$A$为空间的包含集合类$\mathscr{G}\cap A$的任意一个$\sigma$-代数, 下面证明$ \left(\sigma(\mathscr{G})\cap A\right)\subset \mathscr{E}$.
		    因为$\mathscr{E}$包含集合类$\mathscr{G}\cap A$, 所以$\forall B\in\mathscr{G}, AB\in\mathscr{E}$.
		    令\begin{equation}
		        \mathscr{H}_A = \{H:AH\in\mathscr{E}\}.
		    \end{equation}
	...
	\begin{homeworkProblem}
		...
		\solution
		\begin{equation}
		    \sigma(\mathscr{G}) = \mathscr{G}\cup\{ G\cup(\Omega\backslash A):G\in\mathscr{G} \}.
		\end{equation}
	\end{homeworkProblem}
	...
	\begin{homeworkProblem}
		...
		\end{enumerate}
        \textbf{分析:}
		    回忆数学分析中引入的"映射的逆像"(preimage/inverse image).
            ...
            \begin{proof}
                ...
                \begin{align}
                    ...
                    &\Longleftrightarrow \varphi(x)\notin D\nonumber\\
                    &\Longleftrightarrow\varphi(x)\notin D^c\nonumber\\
                    &\Longleftrightarrow x\in \varphi^{-1}(D^c)
                \end{align}
            \end{proof}
            利用这两个公式, 就容易完成本题的证明.
            \begin{proof}
